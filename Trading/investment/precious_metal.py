...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Investment tracker for precious metals"
    )
    parser.add_argument(
        "--path",
        type=str,
        help="Path to the json investment file",
        default="silver.json",
    )
    parser.add_argument(
        "--current_market_price_g",
        type=float,
        help="Current market price in EUR per gram",
    )
    ...
    investments = PreciousMetalInvestments(
        [PreciousMetalInvestment(**entry) for entry in data]
    )
