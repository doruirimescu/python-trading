...
class GurufocusAnalysis(BaseModel):
    ...
    profitability_rank: Optional[float] = None
...
def extract_stock_info(soup: BeautifulSoup) -> GurufocusAnalysis:
    ...
    if title_tag:
        ...
        if "(" in title_text and ")" in title_text:
            ...
            name_part, ticker_part = parse_name_ticker(title_text)
            data["company_name"] = name_part.strip()
            data["ticker"] = ticker_part.strip()
    ...
    # --- Helper to pull a X/10 style rank that sits in the same header row as an <h2> ---
    def extract_header_rank(header_text: str):
        for h2 in soup.find_all("h2"):
            if header_text in h2.get_text(strip=True):
                # The numeric rank is in the same "flex flex-center" container as the small header bar
                container = h2.find_parent().find("div", class_="flex flex-center")
                # Fallback: look forward in the DOM if parent link varies
                if not container:
                    container = h2.find_next("div", class_="flex flex-center")
                if not container:
                    continue
                score_span = container.find("span", class_="t-default bold")
                if score_span:
                    raw = score_span.get_text(strip=True)
                    # Expect forms like "3" or "3/10" -> take the first number
                    try:
    ...
    # --- NEW: Profitability / Growth / Momentum Ranks ---
    data["profitability_rank"] = extract_header_rank(
        "Profitability Rank"
    )  # e.g., 3/10 in your HTML
    data["growth_rank"] = extract_header_rank("Growth Rank")  # e.g., 2/10
...
def parse_name_ticker(title_text: str):
    """
    Returns (name, ticker) where ticker is taken from the last (...) group.
    If no (...) exists, returns (title_text.strip(), None).
    """
    m = LAST_PARENS.search(title_text)
    if not m:
        return title_text.strip(), None

    ticker = m.group(1).strip()
    # Everything before the last (...) is the name; trim common trailing separators/spaces
    name = title_text[: m.start()].rstrip(" -|").strip()
    return name, ticker
...
class GurufocusAnalyzer(StatefulDataProcessor):
    ...
    def process_item(self, item: str, soup: BeautifulSoup | str, iteration_index):
        self.logger.info(
            f"Item: {item}, soup length: {len(str(soup)) if soup else 'None'}"
        )
        ...
        try:
            ...
            self.logger.error(f"Error extracting stock info for item {item}: {e}")
            return
