...
def _get_p_increments(prices: List[float]):
    price_percentage_increments = [
        100 * prices[i+1] / prices[i] if prices[i] != 0 else 0
        for i in range(len(prices)-1)
        ]
    return price_percentage_increments

def count_probability_p_increment(prices: List[float], p: int):
   # probability that the price will increase by p% or more
   # p is a percentage expressed from 0 to 100
    n = 0
    price_percentage_increments = _get_p_increments(prices)
